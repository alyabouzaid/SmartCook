...
export default function formReducer(state = initialState, action) {
    switch (action.type) {
        ...
        case 'RECIPE_ANNOTATION':
            return {
                ...state, title: action.payload.label, images: [{secure_url: action.payload.image}],
                body: parseRecipe(action.payload), initialData: parseRecipe(action.payload)
            };
        case 'JOURNAL_IMPORT':
            return {
                ...state, _id: action.payload._id, author: action.payload.author, title: action.payload.title,
